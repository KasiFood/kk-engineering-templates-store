...
const { computeItnSignature } = require("./payfast-signature");
...
function extractProductSlug(mPaymentId) {
  ...
  const currentMatch = value.match(/^KK-([a-z0-9-]+)-\d{10,}-[a-f0-9]{16}$/);
  if (currentMatch) return currentMatch[1];

  const legacyMatch = value.match(/^KK-([a-z0-9-]+)-\d+$/);
    ...
    content: [{
      ...
      value: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;padding:2rem;background:#0A0C10;color:#F0EEE8;border-radius:8px;">
          ...
          <p style="color:#52555E;font-size:12px;">This link is single-use and expires in 48 hours. If you have any issues, reply to this email.</p>
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Do not hard-code a small PayFast IP list here. PayFast infrastructure can change.
    // Security is enforced by: ITN signature check, PayFast server validation, merchant ID check,
    // payment status check, and exact amount check.
    if (!pfData.signature) {
      console.error("Missing ITN signature.");
      return { statusCode: 400, body: "Missing signature" };
    }

    const expectedSignature = computeItnSignature(pfData, passphrase);
    ...
    if (String(pfData.merchant_id || "").trim() !== merchantId) {
